...
function AdminRoomTransfersPage() {
  ...
  async function loadRequests() {
    try {
      ...
      const data = await apiGet(`/requests/room-transfers${query}`, { token });
  ...
  async function handleUpdateStatus(newStatus) {
    ...
    try {
      ...
      await apiPatch(
        `/requests/room-transfers/${selectedRequest.id}`,
        {
          status: newStatus,
  ...
  async function handleDelete(id) {
    ...
    try {
      setDeletingId(id);
      await apiDelete(`/requests/room-transfers/${id}`, { token });
    ...
    <main className="main-grid">
      <section className="panel">
        <div className="panel-header">
          ...
          <h1 className="page-title">Room Transfer Requests</h1>
          <p>Review and process guest requests to change rooms.</p>
        </div>
        ...
        <div className={filteredRequests.length ? 'room-list' : 'room-list empty-state'}>
          {loading ? 'Loading requests...' : filteredRequests.length ? filteredRequests.map((request) => (
            ...
            >
              <header>
                <div>
                  ...
                  <p className="room-meta">
                    {request.from_room_number || 'Room ' + request.from_room_id} → {request.to_room_number || 'Room ' + request.to_room_id}
                  </p>
                </div>
                <span className="tag">{
              ...
              <p className="room-meta">{formatDateTime(request.created_at)}</p>
            </article>
          )) : 'No requests found.'}
        </div>
      </section>
...
export default AdminRoomTransfersPage;
