...
function AdminRefundRequestsPage() {
  ...
  async function loadRequests() {
    try {
      ...
      const data = await apiGet(`/requests/refund-requests${query}`, { token });
  ...
  async function handleDelete(id) {
    ...
    try {
      setDeletingId(id);
      await apiDelete(`/requests/refund-requests/${id}`, { token });
    ...
    <main className="main-grid">
      <section className="panel">
        <div className="panel-header">
          ...
          <p>Review and process guest refund requests.</p>
        </div>
            ...
            >
              <header>
                <div>
                  ...
                  <p className="room-meta">Amount: {formatPeso(request.requested_amount)}</p>
                </div>
                <span className="tag">{
        ...
        {selectedRequest ? (
          <section className="panel">
            ...
            <div className="summary-card">
              ...
              <div><strong>Amount:</strong> {formatPeso(selectedRequest.requested_amount)}</div>
              <div><strong>Status:</strong> {
                statusOptions.find(o => o.value === selectedRequest.request_status)?.label || selectedRequest.request_status
              }</div>
              <div><strong>Submitted:</strong> {formatDateTime(selectedRequest.created_at)}</div>
            </div>
